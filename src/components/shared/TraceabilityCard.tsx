...
export function TraceabilityCard({
  ...
  return (
    <div className="flex items-center gap-2 rounded-2xl border border-border bg-card p-4 overflow-x-auto">
      <Node
        icon={<Leaf className="w-4 h-4 text-stone" />}
        label={wasteSupplierName}
        sublabel="Waste Source"
      />
      <Connector />
      <Node
        icon={<Store className="w-4 h-4 text-forest" />}
        label={junkShopName}
        sublabel="Junk Shop"
      />
      <Connector />
      <Node
        icon={<Hammer className="w-4 h-4 text-terracotta" />}
        label={artisanName}
        sublabel="Artisan"
      />
    </div>
...
function Node({
  ...
  return (
    <div className="flex flex-col items-center gap-1 min-w-18 max-w-22">
      <div className="w-9 h-9 rounded-full bg-muted flex items-center justify-center shrink-0">
        {icon}
      </div>
      <span className="text-[11px] font-semibold text-charcoal text-center leading-tight line-clamp-2">
        {label}
      </span>
...
function Connector() {
  return (
    <div className="flex-1 flex items-center min-w-5">
